...
# --- CONFIGURAZIONE ---
st.set_page_config(page_title="Sniper V78 - New Format", page_icon="🧬", layout="wide")
st.title("🧬 Sniper Bet V78 (New Format Reader)")
st.markdown("""
**Versione aggiornata per il nuovo formato dati:**
Legge automaticamente le colonne `Place 1` e `Place 2` ed estrae la classifica tra parentesi `(...)`.
""")
st.markdown("---")
...
def calc_hybrid(row, base_hfa, dyn, strat1, strat2):
    ...
    try:
        ...
        # HFA Dinamico (Usa le nuove colonne estratte)
        curr_hfa = base_hfa
        if dyn:
            # Cerchiamo le colonne normalizzate 'rank_h' e 'rank_a'
...
def load_data(file, hfa, dyn, s1, s2):
    try:
        # Tenta lettura automatica separatore
        try:
        ...
        ren = {
            ...
            # Mappiamo le colonne grezze del nuovo formato
            'place 1': 'raw_place_1', 'place1': 'raw_place_1',
            'place 2': 'raw_place_2', 'place2': 'raw_place_2',
            # Manteniamo supporto per vecchio formato
        ...
        if 'cotaa' not in df.columns:
            return None, f"Colonne quote non trovate. Colonne nel file: {list(df.columns)}"

        df = df.dropna(subset=['cotaa'])
        
        # --- 2. ESTRAZIONE CLASSIFICA (LOGICA NUOVA) ---
        # Se abbiamo le colonne grezze 'raw_place_1' (formato nuovo), estraiamo il numero tra parentesi
        if 'raw_place_1' in df.columns:
            # Estrae cifre \d+ che sono dentro parentesi \( ... \)
...
# --- MAIN APP ---
uploaded = st.file_uploader("Carica File CSV (Nuovo Formato)", type=["csv"])
        ...
        if not targets.empty:
            ...
            if has_results:
                ...
                with c1:
                    ...
                    if tot_l1 > 0:
                        st.write(f"Pareggi (X): **{draws1}** | Sconfitte Nette: **{direct1}**")

                tot_l2, draws2, direct2 = analyze_losses(t_s2)
                with c2:
                    st.info(f"Dettaglio Perse S2: {tot_l2}")
                    if tot_l2 > 0:
                        st.write(f"Pareggi (X): **{draws2}** | Sconfitte Nette: **{direct2}**")
                ...
                # Check HFA/Rank nella tabella
                # Se abbiamo estratto rank_h_home, mostriamolo per debug se vuoi, altrimenti teniamo HFA
