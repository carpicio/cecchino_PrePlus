...
# --- CONFIGURAZIONE ---
st.set_page_config(page_title="Sniper V75 - Crash Proof", page_icon="🛡️", layout="wide")
st.title("🛡️ Sniper Bet V75 (Stable & Robust)")
st.markdown("""
Versione corretta per gestire file con colonne mancanti o nomi diversi.
""")
st.markdown("---")
...
def load_data(file, hfa, dyn, s1, s2):
    try:
        ...
        # MAPPING ESTESO (Per evitare errori se cambiano i nomi)
        ren = {
    ...
    if df is not None:
        ...
        if not targets.empty:
            ...
            if has_results:
                ...
                # FIX ANTI-CRASH: Filtra solo le colonne che esistono davvero
