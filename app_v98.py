...
def analyze_segment(df_seg, label):
    ...
    c_roi1.markdown(f"📉 **Bet 1**\n\nProfit: {pnl_1:.2f}u\n\nROI: {roi_1:.1f}%")
    c_roi2.markdown(f"😐 **Bet X**\n\nProfit: {pnl_x:.2f}u\n\nROI: {roi_x:.1f}%")
    c_roi3.markdown(f"🏆 **Bet 2**\n\nProfit: {pnl_2:.2f}u\n\nROI: {roi_2:.1f}%")
        ...
        if df_stud is not None:
            ...
            if not targets_s.empty:
                if 'Real_Res' in targets_s.columns and targets_s['Real_Res'].ne('-').any():
                    ...
                    # CORREZIONE: Pick_Code aggiunto alla lista colonne
                    cols = ['Signal', 'txtechipa1', 'txtechipa2', 'Pick', 'Pick_Code', 'Quota', 'Real_Res', 'Correct_Score', 'EV']
