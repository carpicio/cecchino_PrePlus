...
# --- AUTO-INSTALLAZIONE LIBRERIE MANCANTI ---
# Questo blocco prova a installare openpyxl se manca, per leggere i file .xlsx
try:
    import openpyxl
except ImportError:
    try:
        ...
        pass # Se fallisce, lo gestiremo dopo con un messaggio errore
...
# --- CONFIGURAZIONE ---
st.set_page_config(page_title="Sniper V81 - Auto Install", page_icon="💾", layout="wide")
st.title("💾 Sniper Bet V81 (Excel Fix)")
st.markdown("""
Questa versione prova a installare automaticamente i componenti necessari per leggere Excel (`openpyxl`).
Se continua a non funzionare, salva il tuo file come **CSV (MS-DOS)** da Excel.
""")
st.markdown("---")
...
def load_data(file, hfa, dyn, s1, s2):
    try:
        ...
        if filename.endswith('.csv'):
            ...
            try:
                # Prova a leggere Excel. Se manca la libreria, pandas lancerà errore
                df = pd.read_excel(file)
            except ImportError as e:
                return None, f"⚠️ Manca il componente per leggere Excel ({e}).\n\nSOLUZIONE: Apri il file con Excel e salvalo come **CSV (delimitato da separatore)**, poi carica quello."
            except Exception as e:
                return None, f"Errore lettura file: {str(e)}"
        else:
            return None, "Formato non valido. Carica un CSV o Excel."
        ...
        if 'cotaa' not in df.columns:
            return None, f"Colonne quote non trovate. Trovate: {list(df.columns)}"
        ...
        if 'raw_place_1' in df.columns:
            ...
            df['rank_h_home'] = df['rank_h_home'].fillna(df['raw_place_1']) # Fallback
        ...
        if 'raw_place_2' in df.columns:
            ...
            df['rank_a_away'] = df['rank_a_away'].fillna(df['raw_place_2']) # Fallback
